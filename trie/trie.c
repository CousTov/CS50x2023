...
bool check(char *word)
{
    // Setting node pointer to the root of the trie
    node *cursor = root;

    // Iterating through each letter of char *word
    for (int i = 0; word[i] != '\0'; i++)
    {
        // Index of each letter of the word, without case sensitivity
        int letterIndex = tolower(word[i]) - 'a';

        // If indexed letter(s) from input word is not found
        if (cursor -> children[letterIndex] == NULL)
        {
            return false;
        }

        // If indexed letter(s) from input word is found, go to next
